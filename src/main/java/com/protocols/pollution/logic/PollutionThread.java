...
public class PollutionThread extends Thread{
	...
	void move(int x, int y) throws LocationNotReadyException {
		Double xTarget = copter.getLocationUTM().getX() + 10;//PollutionParam.origin.x + (x * PollutionParam.density);
		Double yTarget = copter.getLocationUTM().getY() + 10;//PollutionParam.origin.y + (y * PollutionParam.density);
		gui.log("moving to " + xTarget + " " + yTarget);
		//copter.moveTo(xTarget,yTarget,PollutionParam.altitude);
