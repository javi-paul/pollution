...
public class PollutionThread extends Thread{
	...
	void move(int x, int y) throws LocationNotReadyException {
		Double xTarget = copter.getLocationUTM().getX() + x;//PollutionParam.origin.x + (x * PollutionParam.density);
		Double yTarget = copter.getLocationUTM().getY() + y;//PollutionParam.origin.y + (y * PollutionParam.density);
	...
	public void run() {
        ...
        int dx = 1;
        int dy = 0;
        for (int i = 1; xpos <= 39 && xpos >= 0 && ypos <= 39 && ypos >= 0; i++) {
            ...
            for (int j = 1; !circle; j++) {
                ...
                ypos += dy;
                try {
                	moveAndRead(new Point(dx*10,dy*10));
				} catch (Exception e) {
					// TODO: handle exception
				} 
               // System.out.println(xpos + " , " + ypos);

                if (xpos >= 39 || xpos < 0 || ypos >= 39 || ypos < 0) {
                    circle = true;
                //    border = true;
                }
            }
